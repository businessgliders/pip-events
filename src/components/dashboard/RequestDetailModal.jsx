...
import { X, Mail, Phone, Trash2, ClipboardList, MessageSquare } from 'lucide-react';
...
export default function RequestDetailModal({ request, onClose, onUpdate }) {
  ...
  const [activeTab, setActiveTab] = useState('details'); // 'details' | 'email'
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div
        ...
        style={{maxWidth: '880px', maxHeight: '90vh', overflow: 'hidden'}}
        onClick={e => e.stopPropagation()}
      >
        {/* ── Top bar ── */}
        <div className="flex items-center justify-between px-6 py-4 flex-shrink-0"
          style={{borderBottom: '1px solid rgba(247,177,189,0.3)', background: 'linear-gradient(135deg, rgba(251,224,226,0.4), rgba(255,255,255,0.9))'}}>
          <div>
            <h2 className="text-lg font-bold" style={{color: '#6b4e4e'}}>{request.full_name}</h2>
            <p className="text-xs mt-0.5" style={{color: '#c48a96'}}>{request.event_type} · {request.event_date ? format(new Date(request.event_date + 'T12:00:00'), 'MMMM d, yyyy') : '—'}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="flex rounded-xl overflow-hidden" style={{border: '1.5px solid rgba(220,200,205,0.6)'}}>
              <button
                ...
                style={{
                  background: activeTab === 'details' ? 'linear-gradient(135deg, #f1889b, #e86c84)' : 'white',
                  color: activeTab === 'details' ? 'white' : '#b67651',
                }}
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold transition-all"
                style={{
                  background: activeTab === 'email' ? 'linear-gradient(135deg, #f1889b, #e86c84)' : 'white',
                  color: activeTab === 'email' ? 'white' : '#b67651',
                  borderLeft: '1px solid rgba(220,200,205,0.6)',
                }}
              >
                <MessageSquare className="w-3.5 h-3.5" /> Emails
                {(request.email_log?.length || 0) > 0 && (
                  <span className="ml-1 rounded-full w-4 h-4 flex items-center justify-center text-xs font-bold"
                    style={{background: activeTab === 'email' ? 'rgba(255,255,255,0.3)' : 'rgba(241,136,155,0.15)', color: activeTab === 'email' ? 'white' : '#e86c84', fontSize: '10px'}}>
                    {request.email_log.length}
                  </span>
                )}
              </button>
            </div>
        ...
        {/* ── Footer ── */}
        <div className="flex-shrink-0 px-6 py-3 flex gap-2" style={{borderTop: '1px solid rgba(247,177,189,0.25)'}}>
          <button onClick={() => setActiveTab('email')}
            className="flex-1 flex items-center justify-center gap-2 border hover:border-pink-200 text-gray-600 hover:text-pink-500 py-2 rounded-xl text-sm font-medium transition-colors"
            style={{borderColor: 'rgba(220,200,205,0.7)'}}>
            <Mail className="w-4 h-4" /> Email Client
          </button>
          {request.phone && (
            <button onClick={() => window.open(`tel:${request.phone}`)}
              className="flex-1 flex items-center justify-center gap-2 border hover:border-blue-200 text-gray-600 hover:text-blue-500 py-2 rounded-xl text-sm font-medium transition-colors"
              style={{borderColor: 'rgba(220,200,205,0.7)'}}>
